...
if __name__ == "__main__":
    ...
    icd_code, meds = get_medications_for_disease(disease, top_n=10)

    if meds:
        print(f"\nTop {len(meds)} Most Prescribed Medications:")
        for i, (name, count) in enumerate(meds, 1):
            print(f"  {i}. {name} ({count:,} FDA reports)")
    elif icd_code:
